...
export default function InstructorSignup() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    setServerError("");
    try {
      const res = await fetch(`${BACKEND}/instructor/signup`, {
        method: "POST",
  ...
  if (pendingEmail) return (
    <div className="min-h-screen flex items-center justify-center bg-white px-6">
      <div className="w-full max-w-md text-center space-y-5">
        <div className="w-16 h-16 rounded-full bg-purple-100 flex items-center justify-center mx-auto text-3xl">✉️</div>
        <h2 className="text-2xl font-bold text-gray-900">Check your inbox</h2>
        <p className="text-sm text-gray-500 leading-relaxed">
          We sent a verification link to <strong className="text-gray-700">{pendingEmail}</strong>.
          Click it to activate your instructor account.
        </p>
        <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 text-xs text-purple-700 text-left space-y-1">
          ...
          <p>• After verifying, sign in at the regular login page.</p>
        </div>
        <p className="text-sm text-gray-400">
          <Link href="/login" className="text-purple-600 font-medium hover:underline">Go to Sign in</Link>
        </p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-[45%] flex-col items-center justify-center bg-[#1E1347] relative overflow-hidden px-12">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <p className="mt-4 text-purple-200/55 text-[14px] leading-relaxed max-w-[280px]">
            Review pending student registrations and approve access to SynthCS for your class.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-white px-8 py-12">
        ...
        <div className="w-full max-w-md">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Create instructor account</h2>
            <p className="text-sm text-gray-500 mt-1">Register as an instructor to manage student approvals</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input
                {...register("email")}
                type="email"
                placeholder="instructor@gordoncollege.edu.ph"
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            ...
            <div className="space-y-2 pt-1">
              <div className="flex items-start gap-2">
                <input {...register("agreeTerms")} type="checkbox" id="agreeTerms" className="mt-0.5 accent-purple-600" />
                <label htmlFor="agreeTerms" className="text-sm text-gray-600 cursor-pointer">
                  I have read and agree to the <span className="text-purple-600 font-medium">Terms of Service</span>
                </label>
              </div>
              {errors.agreeTerms && <p className="text-xs text-red-500 pl-5">{errors.agreeTerms.message}</p>}

              <div className="flex items-start gap-2">
                <input {...register("agreePrivacy")} type="checkbox" id="agreePrivacy" className="mt-0.5 accent-purple-600" />
                <label htmlFor="agreePrivacy" className="text-sm text-gray-600 cursor-pointer">
                  I have read and agree to the <span className="text-purple-600 font-medium">Privacy Policy</span>
                </label>
              </div>
