      ...
      {loading && (
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm animate-pulse">
          <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 grid grid-cols-4 gap-4">
            {["w-24","w-12","w-16","w-14"].map((w,i) => <div key={i} className={`h-3 ${w} bg-gray-200 rounded`} />)}
          </div>
          {[...Array(5)].map((_,i) => (
            <div key={i} className="px-4 py-3 border-b border-gray-50 grid grid-cols-4 gap-4 items-center">
              <div className="flex items-center gap-2">
